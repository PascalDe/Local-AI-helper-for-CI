...
├── pi-scripts/
│   ├── chromadb/
│   │   ├── setup_chroma.py          # Creates the ChromaDB collection "SAP_solutions"
│   │   ├── insert_sap_note.py       # Inserts a dummy SAP note
│   │   ├── insert_sap_blog.py       # Inserts a dummy SAP blog post
│   │   └── check_chroma.py          # Verifies collection contents
│   ├── fastapi/
│   │   └── app/
│   │       └── main.py              # FastAPI app with /health and /analyze-error endpoints
│   └── llm_model/
│       └── download_model.py        # Downloads TinyLlama from HuggingFace
└── llm-ui/
...
### Step 1 – Initial Setup on the Raspberry Pi
...
### Step 4 – LLM Setup (llama.cpp + TinyLlama)
...
The model used is `tinyllama-1.1b-chat-v0.3.Q4_K_M.gguf` – a medium, balanced quality model from [Hugging Face](https://huggingface.co/TheBloke/TinyLlama-1.1B-Chat-v0.3-GGUF), licensed under **Apache 2.0**.
...
While it's not on the level of ChatGPT, it runs entirely on a single-board computer with no licensing fees – and that's the point. Feeding the databases with more accurate, real-world information will improve results further.
